...
import { AddBox, AppsOutlined, BusinessCenter, Chat, Group, HomeRounded, Notifications, SearchRounded } from '@mui/icons-material'
		...
		<>
			<header className='md:sticky top-0 z-50 flex items-center md:justify-between lg:justify-around h-14 w-full bg-white shadow-sm'>
				...
				<div className='flex items-center  w-full md:min-w-max md:max-w-xs lg:max-w-sm h-full space-x-2 px-2'>
					<Link to="/">
						<img src="/images/linkedin.png" alt="logo" className='hidden md:flex h-8' />
					</Link>
					...
					<div className='flex items-center flex-1 bg-[#eef3f8] px-2 h-9'>
						...
						<input type="text" placeholder='Search' className='bg-transparent py-1 px-2 w-[100%] outline-none border-none' />
					</div>
					<Chat className='flex md:!hidden text-gray-500' />
				</div>

				{/* right */}
				<nav className='absolute bottom-0 px-2 bg-white shadow-md md:shadow-none md:relative w-[100vw] md:w-max  flex items-center h-12 md:justify-end md:divide-x divide-gray-300'>
					<div className='flex items-center h-12 justify-between w-full md:justify-normal md:space-x-10 lg:space-x-6 md:px-4'>
						...
						>
							<HeaderLink Icon={Avatar} title="Me" hidden onClick={onClickAvatar} />
							{showSingOutBtn &&
								<button
									onClick={signOut}
